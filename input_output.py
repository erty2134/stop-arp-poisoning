...
class CommandSerialization:
    ...
    """
    @staticmethod
    def _parse_input(input_: str)->list[str,str,str]:
        """doesnt parse value"""
...
def targetip(statement, command, value) -> int: # return is the error status, 0 is no error, may REMOVE!
    if statement == "set":
...
def targetmac(statement, command, value) -> int:
    if statement == "set":
        print("set mac")

while True:
    inputComands.update(input_ = input("in: "))